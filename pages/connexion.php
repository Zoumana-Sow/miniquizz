<div class="contenair content">
         <div class="connexion">
             <p>Connexion</p>
         </div>

         <div class="row content-form">
            <form action="" method="post" id="form-connexion">
               <div class="input-form">
                      <div class="texte">Pseudo</div>
                     <div class="icon-form"></div>
                     <input type="text" class="form-control" error="error-1" name="login">
                     <div class="error-form" id="error-1"></div>
               </div>
               <div class="input-form">
                     <div class="text">Mot de passe</div>
                     <div class="pass-form"></div>
                     <input type="password" class="form-control" error="error-2" name="pwd">
                     <div class="error-form" id="error-2"></div>
               </div>
               <div class="input-form">
                
                      <button type="submit" class="btn-form" name="buton_submit"> </button>
                      <svg class="bi bi-arrow-right-square-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm5.646 10.646a.5.5 0 0 0 .708.708l3-3a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L9.793 7.5H5a.5.5 0 0 0 0 1h4.793l-2.147 2.146z"/>
</svg>
                      <a href="" class="link-form">S'inscire pour jouer?</a>
               </div> 
            </form>



         </div>
         

                 








</div>